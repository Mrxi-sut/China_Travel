...
if __name__ == "__main__":
    model_dir = (Path(__file__).resolve().parent / "model").as_posix()
    cmd = [
             "vllm", "serve", model_dir,
             "--api-key", os.getenv("VLLM_API_KEY", "abc123"),
             "--served-model-name", os.getenv("VLLM_MODEL_NAME", "Qwen3-8B"),
             "--max-model-len", "32768",
             "--port", os.getenv("VLLM_PORT", "7890"),
              ]
    log = open("/tmp/vllm.log", "a")
